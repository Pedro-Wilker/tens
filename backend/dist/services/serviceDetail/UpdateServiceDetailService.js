...
exports.UpdateServiceDetailService = void 0;
...
class UpdateServiceDetailService {
    execute(_a) {
        return __awaiter(this, arguments, void 0, function* ({ serviceId, photoDetails }) {
            ...
            const existingDetails = yield prisma_1.default.serviceDetail.findMany({
                where: { serviceId },
            });
            for (const detail of existingDetails) {
                const oldImagePath = path_1.default.join(uploadDir, detail.photoUrl);
                if (fs_1.default.existsSync(oldImagePath)) {
                    fs_1.default.unlinkSync(oldImagePath);
                }
            }
            yield prisma_1.default.serviceDetail.deleteMany({
                where: { serviceId },
            });
            const updatedDetails = yield Promise.all(photoDetails
                .filter((photo) => photo.file)
                .map((photo) => prisma_1.default.serviceDetail.create({
                data: {
                    serviceId,
...
exports.UpdateServiceDetailService = UpdateServiceDetailService;
